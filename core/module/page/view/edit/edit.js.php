...
function buildPagesList(extraPosition) {
    const hierarchy = JSON.parse($("#pageEditDataContainer").data("hierarchy"));
    const pages = JSON.parse($("#pageEditDataContainer").data("pages"));
    ...
    const messageNone = $("#pageEditDataContainer").data("translate-none");
    const messageBegin = $("#pageEditDataContainer").data("translate-begin");
    const messageAfter = $("#pageEditDataContainer").data("translate-after");

    positionDOM.empty().append(
        $("<option>").val(0).text(messageNone),
        $("<option>").val(1).text(messageBegin)
    );
    ...
    if (parentSelected === "") {
            ...
            if (hierarchy.hasOwnProperty(key)) {
                if (key === currentPage) {
                    ...
                    if (extraPosition == pages[key].extraPosition) {
                        positionPrevious++;
                        positionDOM.append(
                            $("<option>").val(positionPrevious).html(`${messageAfter} "${pages[key].title}"`)
                        );
                    }
                }
            }
        ...
        for (let i = 0; i < hierarchy[parentSelected].length; i++) {
            if (hierarchy[parentSelected][i] === currentPage) {
                ...
                positionPrevious++;
                positionDOM.append(
                    $("<option>").val(positionPrevious).html(`${messageAfter} "${pages[hierarchy[parentSelected][i]].title}"`)
                );
            }
        }
    }
