...
class download extends common
{
	...
	public function comment()
	{
		...
		for ($i = $pagination['first']; $i < $pagination['last']; $i++) {
			...
			self::$comments[] = [
				helper::dateUTF8('%d %B %Y - %H:%M',$comment['createdOn']),
				$comment['content'],
	...
	public function config()
	{
		...
		for ($i = $pagination['first']; $i < $pagination['last']; $i++) {
			...
			// Met en forme le tableau
			$date = helper::dateUTF8('%d %B %Y', $this->getData(['module', $this->getUrl(0), 'posts', $itemIds[$i], 'versionDate']));
	...
	public function stats()
	{
		...
		for ($i = $pagination['first']; $i < $pagination['last']; $i++) {
			...
			$date = helper::dateUTF8('%d %B %Y - %H:%M', $this->getData(['module', $this->getUrl(0), 'posts', $this->getUrl(2), 'stats', $itemIds[$i], 'time']));
			// Met en forme le tableau
			self::$items[] = [
				$date,
	...
	public function list()
	{
		...
		$this->addOutput([
			...
			'content' => json_encode(self::$items)
		]);
	}
